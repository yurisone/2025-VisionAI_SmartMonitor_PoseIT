...
카메라와 IR 센서를 통해 자세 데이터를 수집하고 설정된 임계값과 유지 조건(State Machine)에 따라 3축(전후, 상하, 틸트) 모터를 동기 제어하여 사용자에게 최적의 시청 환경을 물리적으로 제공하도록 구현했습니다.
...
### 1. 비선형 센서 오차 극복을 위한 곡선 피팅 보정 (오차 3% 이하)
...
### 2. 채터링(기계적 마모) 방지를 위한 State Machine 유지 로직 적용
**문제**:
센서 보정 후에도 사용자가 잠깐 물건을 줍는 등의 일시적 움직임에 모터가 민감하게 반응하여 불필요한 구동(채터링)과 기계적 마모가 발생했습니다.
<br><br>
**해결**: 
단순한 Delay 처리가 아닌, 매 루프마다 자세 조건을 재판단해 특정 상태(목각도 50도 이하 등)가 5초 이상 연속 지속될 때만 PWM 제어 명령을 인가하는 상태 유지(State Machine) 시간 필터링 로직을 구현했습니다
